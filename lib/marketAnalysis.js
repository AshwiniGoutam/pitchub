...
export async function analyzeMarketNews(newsItems) {
  try {
    ...
    const newsSummary = newsItems.slice(0, 20).map((item, index) => 
    ...
    Provide your analysis in this exact JSON format without any additional text:
    {
      ...
      "investmentRecommendations": ["specific_recommendation1", "specific_recommendation2", "specific_recommendation3"],
      "emergingThemes": ["theme1", "theme2", "theme3", "theme4", "theme5"],
      "marketSummary": "Comprehensive 3-4 paragraph analysis of current market conditions, focusing on venture capital opportunities, startup ecosystem trends, and strategic investment implications. Include specific sectors showing promise and caution areas."
    }
...
function enhanceAnalysis(analysis) {
  ...
  return {
    ...
    keyTrends: analysis.keyTrends?.length >= 3 ? analysis.keyTrends : getFallbackTrends(),
    emergingThemes: analysis.emergingThemes?.length >= 3 ? analysis.emergingThemes : getFallbackThemes(),
    riskFactors: analysis.riskFactors?.length >= 2 ? analysis.riskFactors : getFallbackRisks()
  };
}
...
function getEnhancedFallbackAnalysis(newsItems) {
  ...
  return {
    ...
    keyTrends: extractedTrends.length >= 3 ? extractedTrends : [
    ...
    sectorOpportunities: [
      ...
      },
      {
        "sector": "HealthTech",
        "outlook": "Positive",
        "reason": "Telemedicine and digital health platforms gaining regulatory support",
        "confidence": "Medium",
        "timeHorizon": "Long-term (18+ months)"
      }
    ],
