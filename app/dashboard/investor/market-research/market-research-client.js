...
export default function MarketResearchClient({ initialData }) {
  ...
  // ✅ React Query handles fetching, caching, and refreshing
  const {
    data: marketData,
    isFetching,
    isLoading,
    ...
    initialData: initialData, // preload from SSR
    staleTime: 10 * 60 * 1000, // 10 mins cache
    refetchOnWindowFocus: false,
  });

  const allNews = marketData?.categorizedNews || [];
  const marketTrends = marketData?.marketTrends || {};
  const investorThesis = marketData?.investorThesis || {};
  const lastUpdated = marketData?.timestamp || new Date().toISOString();
  const error = marketData?.error || false;

  // ✅ Client-side refresh via React Query
  const handleRefresh = async () => {
    await refetch();
  };

  // --- Logic preserved as-is below ---

  const isFutureFocused = (article) => {
    const text = `${article.title} ${article.description}`.toLowerCase();
    return FUTURE_KEYWORDS.some((keyword) =>
  ...
  const ErrorState = () => (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      <p className="text-slate-500 max-w-md mb-4">
        There was an error fetching the latest market data. This might be due to
        CORS restrictions or RSS feed availability.
      </p>
            ...
            <CardContent className="p-6 flex-1 flex flex-col">
              ...
              <div className="flex items-center justify-between mt-auto">
                <div className="flex items-center gap-2">
                  <Button
                    variant="ghost"
                    className="hover:bg-blue-800 text-slate-700 hover:text-slate-200 cursor-pointer p-2 h-auto text-sm group-hover:text-blue-600"
                  >
                    Read More
  ...
  const MarketTrendsSection = () => {
    ...
    if (!marketTrends || marketTrends.trends?.length === 0) {
      return (
        <EmptyState
          message="Analyzing current market trends..."
          icon={TrendingUp}
        />
      );
    }
    ...
    if (!investorThesis.keywords || investorThesis.keywords.length === 0) {
      return (
        <div className="text-center py-8">
          <BarChart3 className="h-12 w-12 text-slate-400 mx-auto mb-4" />
          <h3 className="text-lg font-semibold text-slate-600 mb-2">
            No Investment Keywords Set
          </h3>
          <p className="text-slate-500 mb-4">
            Add positive keywords to your investment thesis to see personalized
            sector analysis.
          </p>
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-slate-50 to-blue-50/30">
      ...
      <div className="flex-1 overflow-auto">
        ...
        <main className="p-8">
          <div className="mb-8">
            <h1 className="text-4xl font-bold text-slate-800">Market Research</h1>
          ...
          {!isFetching && !isLoading && renderActiveSection()}
        </main>
      </div>
    </div>
  );
