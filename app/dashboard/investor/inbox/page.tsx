...
export default function InboxPage() {
  ...
  const calculateRelevanceScore = (email: Email): number => {
    ...
    maxScore += 30;
    if (
      thesisData.sectors?.some((s: string) =>
    ...
    maxScore += 10;
    if (
      thesisData.stages?.some((s: string) =>
  ...
  const acceptPitch = async (emailId: string) => {
    try {
      ...
      const email = emails.find((e) => e.id === emailId?.id);
      if (!email || !emailAnalyses[emailId?.id]) return;
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || "Failed to accept pitch");
      ...
      // ✅ Optionally close sidebar or update local state
                    ...
                    <Card>
                      ...
                      <CardContent>
                        <div className="space-y-3">
                          {emailAnalyses[selectedEmail.id]?.competitiveAnalysis
                            ?.slice(0, 3)
                            .map((point, index) => (
                              <div
                                key={index}
                  ...
                  {loadingDetails[selectedEmail.id] ? (
                    ...
                    <>
                      ...
                      <p className="text-gray-600 text-center max-w-sm mb-4">
                        Sector:{" "}
                        <strong>
                          {emailSectors[selectedEmail.id] || "Unknown"}
                        </strong>
                      </p>
                      <p className="text-sm text-gray-500 mb-6">
                        Click the button below to load comprehensive AI analysis
                        including competitive analysis, market research, and
                        executive summary.
                      </p>
                      <Button
        ...
        >
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Click on any file to download it individually, or use "Download
                All" for all files.
              </DialogDescription>
            </DialogHeader>
                ...
                {downloadingAttachments.size > 0 && (
                  <div className="pt-4 border-t">
                    <div className="flex items-center gap-2 text-sm text-blue-600">
                      ...
                      <span>
                        Downloading {downloadingAttachments.size} file(s)...
                      </span>
                    </div>
                    <Progress
                      value={
                        (downloadedAttachments.size /
                          selectedEmail.attachments.length) *
                        100
                      }
                      className="mt-2"
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {downloadedAttachments.size} of{" "}
                      {selectedEmail.attachments.length} completed
                    </p>
                  </div>
                )}
              </>
